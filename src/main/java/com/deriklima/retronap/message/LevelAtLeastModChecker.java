...
public class LevelAtLeastModChecker extends ConditionChecker {
  ...
  public boolean checkCondition(MessageContext session, Message message) {
    ...
    if (!userLevel.equals(UserLevelTypes.MODERATOR)
        && !userLevel.equals(UserLevelTypes.ELITE)
        && !userLevel.equals(UserLevelTypes.ADMIN)) {
      String errorMsg = "User must be Moderator level or above to execute that command!";
